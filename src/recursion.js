...
var reverse = function(string) {
	...
	if (string.length > 1){
		var result = reverse(string.slice(0, -1));
		result = string[string.length-1] + result;
		return result;
	}
};
...
var rMap = function(array, callback) {
	if (array.length === 1){
		...
		result = [callback(array[0])].concat(result);
		return result;
	}
};
...
var countKeysInObj = function(obj, prop) {
	...
	for (var key in obj){
		...
		if (typeof obj[key] === 'object'){
			count += countKeysInObj(obj[key], prop);
		}
	}
	return count;
};
...
var capitalizeWords = function(array) {
	if (array.length === 1){
		...
		result = [array[0].toUpperCase()].concat(result);
		return result;
	}
};
...
var capitalizeFirst = function(array) {
	if (array.length === 1){
		...
		result = [array[0][0].toUpperCase() + array[0].slice(1)].concat(result);
		return result;
	}
};
